...
import { dataProcess } from "./Tools";

const HilbertCurveStrategy_GetOrderedValues = (unsorted) => {
    ...
    for (let x = 0; x < unsorted.length; x++) {
        ...
        for (let y = 0; y < unsorted[x].length; y++) {
            idx[y] = y;
            hilbertValues[y] = encode(parseInt(unsorted[x][y].x), parseInt(unsorted[x][y].y), this.hilbertOrder);
        }
